                ...
                for key, atributo in atributos.items():
                    ...
                    try:
                        ...
                        if valor is not None:
                            ...
                            print(
                                f'{key}: Dado nao encontrado: {ativo.ativoPrincipal}')
                    except:
                        print(
                            f'{ativo.tipo}: Dado nao existente: {ativo.ativoPrincipal} key:{key}')

                # Forma de atribuir a cotação separado das informações, pois o mais importante é a cotacao
                ativo.cotacao = round(ticker.history(
                    period="1d")["Close"].iloc[-1], 2)
                ativo.save()
                print(
                    f'ativo: {ativo.ativoPrincipal} preco: {ativo.cotacao} salvo.')

            else:
                print(
                    f'Não há suporte para {ativo.tipo}: {ativo.ativoPrincipal} no momento.')

        except:
            print(
                f'Erro ao baixar informacoes do ativo: {ativo.ativoPrincipal}')
